...
public class RecordShowActivity extends Activity implements
	...
	private void queryBaiduRawTrace() {
        DbAdapter dbhelper = new DbAdapter(this.getApplicationContext());
        dbhelper.open();
        PathRecord mRecord = dbhelper.queryRecordById(mRecordItemId);
        dbhelper.close();
        try {
            final long startTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").
                    parse(mRecord.getDate()).getTime() / 1000;
            final double duration = Double.valueOf(mRecord.getDuration().
                    substring(0, mRecord.getDuration().length() - 1));
            final long entTime = (long) (startTime + duration);

            TraceHelper.getInstance().queryHistoryTrace(startTime, entTime, new OnTrackListener() {
                @Override
                public void onHistoryTrackCallback(HistoryTrackResponse response) {
                    if (StatusCodes.SUCCESS != response.getStatus()) {
                        Toast.makeText(RecordShowActivity.this, "获取百度轨迹失败", Toast.LENGTH_LONG).show();
                    } else if (0 == response.getTotal()) {
                        Toast.makeText(RecordShowActivity.this, "没有查询到百度轨迹数据", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(RecordShowActivity.this, "获取百度轨迹成功", Toast.LENGTH_LONG).show();
                        final LatLng startPoint = new LatLng(response.getStartPoint().getLocation().getLatitude(),
                                response.getStartPoint().getLocation().getLongitude());
                        final LatLng endPoint = new LatLng(response.getEndPoint().getLocation().getLatitude(),
                                response.getEndPoint().getLocation().getLongitude());
                        final List<LatLng> baiduTrace = baiduPoint2AmapLatLng(response.getTrackPoints());
                        drawBaiduTrace(baiduTrace, startPoint, endPoint, Color.GREEN);
                    }
                }
            }, false);
	...
	private void queryGraspBaiduTrace() {
		DbAdapter dbhelper = new DbAdapter(this.getApplicationContext());
		dbhelper.open();
		PathRecord mRecord = dbhelper.queryRecordById(mRecordItemId);
		dbhelper.close();
				...
				public void onHistoryTrackCallback(HistoryTrackResponse response) {
					if (StatusCodes.SUCCESS != response.getStatus()) {
						...
						final LatLng endPoint = new LatLng(response.getEndPoint().getLocation().getLatitude(),
								response.getEndPoint().getLocation().getLongitude());
                        final List<LatLng> baiduTrace = baiduPoint2AmapLatLng(response.getTrackPoints());
                        drawBaiduTrace(baiduTrace, startPoint, endPoint, Color.RED);
					}
				}
			});
		} catch (ParseException e) {
	...
	private List<LatLng> baiduPoint2AmapLatLng(List<? extends Point> baiduLatLng) {
        List<LatLng> amapLatLng = new ArrayList<>();
        for (Point point: baiduLatLng) {
            final LatLng latLng = new LatLng(point.getLocation().getLatitude(),
                    point.getLocation().getLongitude());
            amapLatLng.add(latLng);
        }
        return amapLatLng;
	}

	private void drawBaiduTrace(List<LatLng> baiduTrace, LatLng startPoint, LatLng endPoint, int lineColor) {
        mOriginPolyline = mAMap.addPolyline(new PolylineOptions().color(
                lineColor).addAll(baiduTrace));
